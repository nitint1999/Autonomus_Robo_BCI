...
TEST_ROW_INDEX = 12 # This is now controlled by the web app
# -----------------------------------
...
def index():
    ...
    with state_lock:
        ...
        test_row_index_locked = TEST_ROW_INDEX # Pass the current index to HTML
    ...
    return render_template_string(html, 
                                 ...
                                 TEST_ROW_INDEX=test_row_index_locked) # Added for HTML initial value
...
# Route to handle TURN speed changes from the slider
...
# NEW ROUTE: To handle updating the test row index
...
def set_test_row():
    ...
    try:
        ...
        # Simple validation: ensure it's non-negative
        new_index = max(0, new_index)
        
        with state_lock:
            TEST_ROW_INDEX = new_index
            
        print(f"✅ TEST_ROW_INDEX updated to: {TEST_ROW_INDEX}") # Debug print
...
<body class="bg-gray-100 p-4 sm:p-8">
    ...
    <script>
        ...
        function sendTestIndexUpdate(index) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // 1. Update the display span to confirm the change
                    document.getElementById('current-test-index').textContent = data.new_index;
                    
                    // 2. IMPORTANT: Update the input field value with the confirmed value
                    document.getElementById('test-row-index-input').value = data.new_index; 

                } else {
                    alert(`Failed to set index: ${data.error}`);
                    console.error("Failed to update index:", data.error);
                }
            })
        ...
        function updateStatus() {
            fetch('/status')
                ...
                .then(data => {
                    ...
                    // Update the displayed current ACTIVE test index (the non-input span)
                    document.getElementById('current-test-index').textContent = data.test_row_index; 
                    
                    // IMPORTANT: DO NOT update the input field value here.
                    // This prevents the status check from overriding the user's typing.
                    // document.getElementById('test-row-index-input').value = data.test_row_index; // <-- REMOVED
        ...
        window.onload = function() {
            ...
            // Set up event listener for the Set Index button
            document.getElementById('set-index-btn').addEventListener('click', function() {
                const inputElement = document.getElementById('test-row-index-input');
                const index = parseInt(inputElement.value);
                if (!isNaN(index) && index >= 0) {
                    sendTestIndexUpdate(index);
                } else {
                    alert("Please enter a valid non-negative integer for the Test Row Index.");
                }
            });
