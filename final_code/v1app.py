from flask import Flask, render_template_string, Response, jsonify, request # ADDED request
import cv2
...
FULL_DUTY_CYCLE = 20    # Initial speed/default value for the slider
TURN_DUTY_CYCLE = 20  # Fixed speed for turning movements
current_duty_cycle = FULL_DUTY_CYCLE # NEW: Variable for user-selected PWM speed
PWM_FREQ = 100 
...
def index():
    ...
    with state_lock:
        ...
        current_duty_cycle_locked = current_duty_cycle # Pass current speed to template
        
    return render_template_string(html, 
                                 current_state=current_state_locked, 
                                 sensor_status=sensor_status,
                                 FULL_DUTY_CYCLE=current_duty_cycle_locked, # Use current_duty_cycle here
                                 TURN_DUTY_CYCLE=TURN_DUTY_CYCLE)

# NEW: Route to handle speed changes from the slider
...
def go_forward():
    global current_state, is_moving, current_duty_cycle
    forward()
    with state_lock:
...
# --- HTML Template (Includes speed slider and 180 degree radar fix) ---
html = """
<!doctype html>
<html lang="en">
<head>
...
<body class="bg-gray-100 p-4 sm:p-8">
    ...
    <div class="max-w-4xl mx-auto bg-white shadow-xl rounded-xl p-6">
        ...
        <p class="text-sm text-gray-500 mb-6 text-center">
            Linear Speed: <span id="initial-speed-disp">{{ FULL_DUTY_CYCLE }}%</span> | Turn Speed: {{ TURN_DUTY_CYCLE }}%
        </p>
        ...
        <div class="flex flex-col items-center space-y-4 mb-8 border-b pb-8">
            ...
            <div class="w-full mb-6 p-4 border border-gray-300 rounded-lg bg-gray-50">
                <label for="speed-slider" class="block text-xl font-semibold text-gray-800 mb-2 text-center">
                    Motor Speed Control: <span id="current-speed-display">{{ FULL_DUTY_CYCLE }}%</span>
                </label>
                <input type="range" 
                       id="speed-slider" 
                       min="0" 
                       max="100" 
                       value="{{ FULL_DUTY_CYCLE }}" 
    ...
    <script>
        ...
        // --- NEW: Speed Slider Control Function ---
        function sendSpeedUpdate(speed) {
            document.getElementById('current-speed-display').textContent = `${speed}%`;
            ...
            .then(data => {
                if (!data.success) {
                    console.error("Failed to update speed:", data.error);
                }
            })
            .catch(error => console.error('Error sending speed update:', error));
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // NEW: Initialize slider and add event listener
            const speedSlider = document.getElementById('speed-slider');
            const initialSpeed = parseInt(document.getElementById('initial-speed-disp').textContent.replace('%', ''));
            speedSlider.value = initialSpeed;
            document.getElementById('current-speed-display').textContent = `${initialSpeed}%`;
