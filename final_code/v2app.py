...
current_duty_cycle = INITIAL_LINEAR_SPEED    # Variable for user-selected linear PWM speed (Forward/Backward)
current_turn_duty_cycle = INITIAL_TURN_SPEED # VARIABLE: For user-selected turn PWM speed (Left/Right/Auto Avoid)
PWM_FREQ = 100 
...
def forward():
    # Uses the linear speed variable
...
def left():
    # Uses the DYNAMIC turn speed variable
    set_speed(current_turn_duty_cycle)
    # Pivot Turn Logic: Stop Left, Forward Right
    GPIO.output(IN1, True)
    GPIO.output(IN2, False)
    GPIO.output(IN3, False) 
    GPIO.output(IN4, False)
...
def right():
    # Uses the DYNAMIC turn speed variable
    set_speed(current_turn_duty_cycle)
    # Pivot Turn Logic: Forward Left, Stop Right
    GPIO.output(IN1, False)
    GPIO.output(IN2, False)
    GPIO.output(IN3, False)
    GPIO.output(IN4, True)
    ...
    try:
        ...
        with state_lock:
            ...
            if "Turning Left" in current_state or "Turning Right" in current_state or "AUTO AVOID" in current_state:
                # Note: This will only update the speed if the turn function is currently running, 
                # but the update logic is handled by the motor control functions on the next call anyway.
                pass 
...
def get_status_json():
    ...
    with state_lock:
        ...
        linear_speed = current_duty_cycle # Pass both speeds for dynamic UI update
...
# --- HTML Template (Includes TWO Speed sliders) ---
html = """
<!doctype html>
<html lang="en">
<head>
...
<body class="bg-gray-100 p-4 sm:p-8">
    ...
    <script>
        ...
        // NEW Function for Turn Speed (Left/Right/Auto Avoid)
        ...
        function updateStatus() {
            fetch('/status')
                ...
                .then(data => {
                    ...
                    // Update speed displays in case they changed via auto-avoidance or button press
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Setup Linear Speed Slider
            const linearSpeedSlider = document.getElementById('linear-speed-slider');
            if (linearSpeedSlider) {
                linearSpeedSlider.addEventListener('input', (event) => {
                    sendLinearSpeedUpdate(event.target.value);
                });
            }

            // Setup Turn Speed Slider (NEW)
            const turnSpeedSlider = document.getElementById('turn-speed-slider');
            if (turnSpeedSlider) {
                turnSpeedSlider.addEventListener('input', (event) => {
                    sendTurnSpeedUpdate(event.target.value);
                });
            }
